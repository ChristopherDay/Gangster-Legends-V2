...
class Template
{
     ...
     */
    public function __construct(Handlebars $engine, $tree, $source, $glTemplate = false)
    {
        $this->glTemplate = $glTemplate;
    ...
    {  
        ...
        $partialName = $current[Tokenizer::NAME];
        ...
        if (isset(
            $this->glTemplate->page->template->$partialName
        )) {
            //$partial = new Template($this->handlebars);

            $html = $this->glTemplate->page->template->$partialName;
            ...
            return $this->handlebars->render($html, $context);
        }

        $partial = $this->handlebars->loadPartial($partialName);
