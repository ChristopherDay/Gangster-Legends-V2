...
/*
 * Originally from Philipp15b
 * Modifications for use in GangsterLegends by uaktags
...
class i18n {
     ...
     * @var array
     */
    protected $filePath = Array('./lang/lang_{LANGUAGE}.ini');
    ...
    protected $langFilePathArray = array();
     ...
     *
     * @param array [$filePath] This is the path for the language files. You must use the '{LANGUAGE}' placeholder for the language.
    ...
    public function init() {
        ...
        foreach ($this->userLangs as $priority => $langcode) 
        {
            ...
            foreach($this->filePath as $languageFile){
                ...
                if($languageFile == $this->filePath[0]){
                    $defaultLangPath = $this->getConfigFilename($langcode, $languageFile);
                    $this->filePath[0] = $defaultLangPath;
                    array_push($this->langFilePathArray, $defaultLangPath);
                }else{
                    $modLang = $this->getConfigFilename($langcode, $languageFile);
                    if(file_exists($modLang)){
                        array_push($this->langFilePathArray, $modLang);
                    }
                }
            }
        ...
        $mdfofLang = "";
        foreach($this->langFilePathArray as $languages){
            $mdfofLang .= md5(filemtime($languages));
        }
        // search for cache file
        $this->cacheFilePath = $this->cachePath . '/php_i18n_' . md5($mdfofLang) . '_' . $this->prefix . '_' . $this->appliedLang . '.cache.php';

        // whether we need to create a new cache file

        $outdated = !file_exists($this->cacheFilePath) ||
            md5(explode('_',$this->cachePath . '/php_i18n_' . md5($mdfofLang) . '_' . $this->prefix . '_' . $this->appliedLang . '.cache.php')[1]) != $this->cacheFilePath; // the fallback language config was updated

        if ($outdated) {
            $config = $this->load($this->langFilePathArray);
            if ($this->mergeFallback)
                $config = array_replace_recursive($this->load($this->getConfigFilename($this->fallbackLang, $this->filePath[0])), $config);
    ...
    protected function getConfigFilename($langcode, $filepath) {
        ...
        foreach($filename as $file){
            ...
            switch ($ext) {
                ...
                case 'yml':
                    ...
                    break;
                case 'json':
                    $config = array_merge($config, json_decode(file_get_contents($file), true));
